...
<tr class="hover:bg-gray-100 text-center">
    ...
    <td class="border px-4 py-2">{{ $kriteria->bobot_kriteria }}</td>
    @if(Auth::user()->role === 'admin')
    <td class="border px-4 py-2">
        <button onclick="openEditModal({{ $kriteria }})" class="text-blue-600 hover:underline">Edit</button>
        <form action="{{ route('kriteria.destroy', $kriteria->id ?? 'kosong') }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin hapus data ini?')">
            @csrf @method('DELETE')
...
@if($kriterias->isEmpty())
    <p class="text-center text-gray-600 mt-4">Belum ada data kriteria. Silakan tambahkan data terlebih dahulu.</p>
@endif
...
<div id="modalTambah" class="fixed inset-0 bg-black bg-opacity-50 justify-center items-center hidden z-50">
    <div class="bg-white p-6 rounded w-full max-w-md">
        ...
        <form action="{{ route('kriteria.store') }}" method="POST">
            @csrf
            <input name="kode_kriteria" placeholder="Kode" class="w-full border p-2 mb-2" required>
            <input name="nama_kriteria" placeholder="Nama Kriteria" class="w-full border p-2 mb-2" required>
            <input name="bobot_kriteria" placeholder="Bobot Kriteria" class="w-full border p-2 mb-2" type="number" step="0.01" required>
...
<div id="modalEdit" class="fixed inset-0 bg-black bg-opacity-50 justify-center items-center hidden z-50">
    <div class="bg-white p-6 rounded w-full max-w-md">
        ...
        <form id="editForm" method="POST">
            @csrf @method('PUT')
            <input name="kode_kriteria" id="editKode" placeholder="Kode" class="w-full border p-2 mb-2" required>
            <input name="nama_kriteria" id="editNamaKriteria" placeholder="Nama Kriteria" class="w-full border p-2 mb-2" required>
            <input name="bobot_kriteria" id="editBobotKriteria" placeholder="Bobot Kriteria" class="w-full border p-2 mb-2" type="number" step="0.01" required>
            <div class="flex justify-end gap-2 mt-4">
